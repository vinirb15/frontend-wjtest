...
import api from '../../mock-api/V1/categories/2';
...
export default function Products() {
  ...
  async function Filter(gender, type) {
    handleLoad()
    setTimeout(() => {
      setFilter(true)
      setObjFilter(obj.filter((asd) => { return asd.filter[0].gender === gender }))
    }, 200)
  }
  ...
  return (
    <>
      <h5 className="mt-3 mb-3 pl-5">Página inicial > <b style={{ color: "var(--bg-color)" }}>Calças</b></h5>
        ...
        <div className="filter">
          ...
          <div>
            ...
            <div class="btn-group-vertical">
              <button className="border-0 ml-1 btn btn-primary" onClick={() => setFilter(false)}>Todos</button>
              <button className="border-0 ml-1 btn btn-primary" onClick={() => Filter('Masculina')}>Masculina</button>
              <button className="border-0 ml-1 btn btn-primary" onClick={() => Filter('Feminina')}>Feminina</button>
            </div>
          </div>
        </div>
